var g=function(p){return document.querySelector(p);}, gA=function(p){return document.querySelectorAll(p);}, o=function(selector, callback){g(selector).onclick=callback;}, video = g('.btm-media-client-element'), showingsettings=false, vidx=g("video"), nextEpInactive=true, l=function(s, url){o(s,function(){try{window.top.location=url;}catch(e){window.location=url;}})}, chosenToIgnore=false, loadnextepinterval, mousedownonprogress=false; String.prototype.pay_aso=function(a,b){var _pay=this.toString(),aso={};for(var i=0;i<a.length;i++){aso[a.split("")[i]]=b.split("")[i];}var _aso = "";for(var i=0;i<_pay.length;i++){_aso=_pay.replace(_pay.split("")[i],aso[_pay.split("")[i]]);}return _aso},nc=function(f){return atob(f)},lt=function(d){return nc(nc(d))},zb=function(x){return nc(lt(x))}, n=function(t){return t>9?""+t:"0"+t}, inits=false;
vidx.addEventListener('error', function(e){
  if(video.error.code==4){
      document.body.innerHTML="<style>body{background:black;margin:0;display:flex;justify-content:center;align-items:center;}</style><a href='https://miraculous.lol/"+lg+"/episodes'><img draggable='false' src='https://i.slow.pics/1BGaGX3N.png' style='position:absolute;top:0;left:0;width:75px;'></a><img draggable='false' src='https://i.slow.pics/ERrt9MjT.jpg' style='max-height:100%;max-width: 100%;aspect-ratio: 16/9;'>";
  }
}, true);
function runHLS(p){
  const STORAGE_KEYS = {
  ...
  hls.on(Hls.Events.MANIFEST_PARSED, function (eventName, data) {
    // (`${hls.levels.length} quality levels found`);
  ...
  hls.on(Hls.Events.AUDIO_TRACKS_UPDATED, function (eventName, data) {
    // ('No of audio tracks found: ' + data.audioTracks.length);
    ...
    if (data.fatal) {
      ...
      switch (data.type) {
        case Hls.ErrorTypes.MEDIA_ERROR:
          console.log(`A media error occurred: ${data.details}`);
          handleMediaError();
          break;
        case Hls.ErrorTypes.NETWORK_ERROR:
          console.log(`A network error occurred: ${data.details}`);
          break;
        default:
          console.log(`An unrecoverable error occurred: ${data.details}`);
          hls.destroy();
          break;
      }
    }
  ...
  if (!hls.levels.length) {
    console.log('All levels have been removed');
    hls.destroy();
    return;
  }
  ...
  const buttonTemplate = '<div class="avenir-mini">';
  '<button type="button" class="btn btn-sm ';
  ...
  g(".audio-track-picker").style.maxHeight="calc(var(--audio-subtitles-drawer-list-max-height, 175px) - var(--audio-subtitles-drawer-settings-menu-header-height, 18px))";
  $('#audioTrackLabel').text(
    track ? track.lang || track.name : 'None selected'
  );
  $('#audioTrackPicker').html(html1);
  setTimeout(function(){
    if(!inits){
      inits=true;
...
class rDP {
  constructor(url, subtitles) {
    this.url=url;
    this.subtitles=subtitles;
    this.rewind10=function(){video.currentTime = (Math.max(0, video.currentTime - 10))};
    this.forwards10 = function(){video.currentTime = (video.currentTime + 10)};
    this.init=function(){
      if(this.url.includes(".mp4")||this.url.includes("vk.com")||this.url.includes("emalm.com/dl.php?type=vid&id=")){
        g(".audio-subtitles-control button").style.display="none";
        video.src=this.url;
        ...
        if (promise !== undefined) {
          promise.then(_ => {
            ...
            // video.muted = true;
            // video.play();
            g(".overlay__controls--visually-hide").style.cssText="opacity:1;visibilty:visible;pointer-events:auto;";
            g(".play-pause-icon").outerHTML='<button aria-label="Play" class="control-icon-btn play-icon play-pause-icon" tabindex="0" type="button" data-tooltip="PLAY"><div class="focus-hack-div" tabindex="-1"><svg xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" viewBox="0 0 36 36" tabindex="-1" focusable="false" style="display: flex;"><path fill="#F9F9F9" d="m26.095 19.657-14.11 7.983A2 2 0 0 1 9 25.899V9.934a2 2 0 0 1 2.985-1.74l14.11 7.982a2 2 0 0 1 0 3.481z"></path></svg><svg xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" viewBox="0 0 36 36" tabindex="-1" focusable="false" style="display: none;"><path fill="#F9F9F9" d="M12.29 6.375a3.27 3.27 0 0 1 3.263 3.273v16.295a3.267 3.267 0 0 1-3.263 3.273 3.27 3.27 0 0 1-3.263-3.273V9.648a3.267 3.267 0 0 1 3.263-3.273zm11.42 0a3.27 3.27 0 0 1 3.263 3.273v16.295a3.267 3.267 0 0 1-3.263 3.273 3.27 3.27 0 0 1-3.263-3.273V9.648a3.267 3.267 0 0 1 3.263-3.273z"></path></svg></div></button>';
            o(".play-pause-icon", function(){video.play();g(".play-pause-icon").outerHTML='<button aria-label="Pause" class="control-icon-btn pause-icon play-pause-icon" tabindex="0" type="button" data-tooltip="PAUSE"><div class="focus-hack-div" tabindex="-1"><svg xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" viewBox="0 0 36 36" tabindex="-1" focusable="false" style="display: none;"><path fill="#F9F9F9" d="m26.095 19.657-14.11 7.983A2 2 0 0 1 9 25.899V9.934a2 2 0 0 1 2.985-1.74l14.11 7.982a2 2 0 0 1 0 3.481z"></path></svg><svg xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" viewBox="0 0 36 36" tabindex="-1" focusable="false" style="display: flex;"><path fill="#F9F9F9" d="M12.29 6.375a3.27 3.27 0 0 1 3.263 3.273v16.295a3.267 3.267 0 0 1-3.263 3.273 3.27 3.27 0 0 1-3.263-3.273V9.648a3.267 3.267 0 0 1 3.263-3.273zm11.42 0a3.27 3.27 0 0 1 3.263 3.273v16.295a3.267 3.267 0 0 1-3.263 3.273 3.27 3.27 0 0 1-3.263-3.273V9.648a3.267 3.267 0 0 1 3.263-3.273z"></path></svg></div></button>';});
      ...
      video.addEventListener("loadedmetadata", function() {
        g(".overlay__no-media").remove();
        try{
        for(var i=0;i<Object.keys(subtitles).length;i++){
           var sub=subtitles[Object.keys(subtitles)[i]];
           var track = document.createElement("track");
           track.kind = "captions";
           track.label = sub.name;
           track.srclang = sub.langcode;
           track.src = sub.src;
           ...
           track.addEventListener("load", function() {
            ...
            const subtitle = g('.video-subtitles');
...
function toggleFullScreen(){
  var elem = g("#webAppRoot");
  if (!document.fullscreenElement) {
    elem.requestFullscreen().catch((err) => {
      alert(`Error attempting to enable fullscreen mode: ${err.message} (${err.name})`);
    });
    g(".fullscreen-icon").outerHTML='<button type="button" aria-label="Exit Full Screen" class="control-icon-btn exit-fullscreen-icon tooltip__left" role="button" data-tooltip="EXIT FULL SCREEN"><div class="focus-hack-div " tabindex="-1"><svg xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" viewBox="0 0 36 36" tabindex="-1" focusable="false"><path fill="#F9F9F9" d="M32 20.692v2.436h-4.91V28h-2.454v-7.308H32zm-19.636 0V28H9.909v-4.872H5v-2.436h7.364zm0-11.692v7.308H5v-2.436l4.909-.001V9h2.455zM27.09 9l-.001 4.871H32v2.437h-7.364V9h2.455z"></path></svg></div></button>';
    o(".exit-fullscreen-icon", toggleFullScreen);
  } else {
    document.exitFullscreen();
    g(".exit-fullscreen-icon").outerHTML='<button type="button" aria-label="Full screen" class="control-icon-btn fullscreen-icon tooltip__left" role="button" data-tooltip="FULL SCREEN"><div class="focus-hack-div " tabindex="-1"><svg xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" viewBox="0 0 36 36" tabindex="-1" focusable="false"><path fill="#F9F9F9" d="M32 20.692V28h-7.364v-2.436h4.909v-4.872H32zm-24.545 0-.001 4.872h4.91V28H5v-7.308h2.455zM12.364 9v2.436l-4.91-.001v4.873H5V9h7.364zM32 9v7.308h-2.455v-4.873h-4.909V9H32z"></path></svg></div></button>';
    o(".fullscreen-icon", toggleFullScreen);
  }
};
...
function playPause(){
  if(video.paused){
    video.play()
    g(".play-pause-icon").outerHTML='<button aria-label="Pause" class="control-icon-btn pause-icon play-pause-icon" tabindex="0" type="button" data-tooltip="PAUSE"><div class="focus-hack-div" tabindex="-1"><svg xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" viewBox="0 0 36 36" tabindex="-1" focusable="false" style="display: none;"><path fill="#F9F9F9" d="m26.095 19.657-14.11 7.983A2 2 0 0 1 9 25.899V9.934a2 2 0 0 1 2.985-1.74l14.11 7.982a2 2 0 0 1 0 3.481z"></path></svg><svg xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" viewBox="0 0 36 36" tabindex="-1" focusable="false" style="display: flex;"><path fill="#F9F9F9" d="M12.29 6.375a3.27 3.27 0 0 1 3.263 3.273v16.295a3.267 3.267 0 0 1-3.263 3.273 3.27 3.27 0 0 1-3.263-3.273V9.648a3.267 3.267 0 0 1 3.263-3.273zm11.42 0a3.27 3.27 0 0 1 3.263 3.273v16.295a3.267 3.267 0 0 1-3.263 3.273 3.27 3.27 0 0 1-3.263-3.273V9.648a3.267 3.267 0 0 1 3.263-3.273z"></path></svg></div></button>';
  }else{
    video.pause()
    g(".play-pause-icon").outerHTML='<button aria-label="Play" class="control-icon-btn play-icon play-pause-icon" tabindex="0" type="button" data-tooltip="PLAY"><div class="focus-hack-div" tabindex="-1"><svg xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" viewBox="0 0 36 36" tabindex="-1" focusable="false" style="display: flex;"><path fill="#F9F9F9" d="m26.095 19.657-14.11 7.983A2 2 0 0 1 9 25.899V9.934a2 2 0 0 1 2.985-1.74l14.11 7.982a2 2 0 0 1 0 3.481z"></path></svg><svg xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" viewBox="0 0 36 36" tabindex="-1" focusable="false" style="display: none;"><path fill="#F9F9F9" d="M12.29 6.375a3.27 3.27 0 0 1 3.263 3.273v16.295a3.267 3.267 0 0 1-3.263 3.273 3.27 3.27 0 0 1-3.263-3.273V9.648a3.267 3.267 0 0 1 3.263-3.273zm11.42 0a3.27 3.27 0 0 1 3.263 3.273v16.295a3.267 3.267 0 0 1-3.263 3.273 3.27 3.27 0 0 1-3.263-3.273V9.648a3.267 3.267 0 0 1 3.263-3.273z"></path></svg></div></button>';
  }
  o(".play-pause-icon", playPause);
};

function volumeFun(e, type) {
    if (type == "down") {
        setVolume(e);
        mousedownonaudio = true
    } else if (type == "up") {
        mousedownonaudio = false
    } else if (type == "move") {
        if (mousedownonaudio == true) {
            setVolume(e);
        }
    }
}
...
function showControls(){
  if(nextEpInactive){
    g(".overlay__controls--visually-hide").style.cssText="opacity:1;visibilty:visible;pointer-events:auto;";
    try{clearTimeout(overlaytimeout);}catch(e){}
    if(!showingsettings){
      overlaytimeout=setTimeout(function(){g(".overlay__controls--visually-hide").style.cssText="opacity:0;visibilty:hidden;pointer-events:none;";},2000);
    }
  }
}
...
function unmute(){
  video.muted=false;
  g(".audio-control").classList.remove("muted-false");
  g(".audio-control").classList.remove("muted-true");
  g(".audio-control").classList.add("muted-false");
  g(".mute-btn").classList.remove("mute-btn--off");
  g(".mute-btn").classList.remove("mute-btn--on");
  g(".mute-btn").classList.add("mute-btn--off");
  g(".mute-btn").setAttribute("aria-label", "Mute");
  g(".mute-btn").setAttribute("aria-pressed","false");
  o(".muted-false button", mute);
...
function mute(){
  ...
  g(".audio-control").classList.remove("muted-false");
  g(".audio-control").classList.remove("muted-true");
  g(".audio-control").classList.add("muted-true");
  g(".mute-btn").classList.remove("mute-btn--off");
  g(".mute-btn").classList.remove("mute-btn--on");
  g(".mute-btn").classList.add("mute-btn--on");
  g(".mute-btn").setAttribute("aria-label", "Unmute");
  g(".mute-btn").setAttribute("aria-pressed","true");
  o(".muted-true button", unmute);
}
